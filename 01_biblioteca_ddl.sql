...
CREATE TABLE Livros (
    ...
    CONSTRAINT FK_Livros_Editoras FOREIGN KEY (id_editora)
    ...
    CONSTRAINT CHK_ano_publicacao CHECK (ano_publicacao >= 1000 AND ano_publicacao <= 2100)
) ENGINE=InnoDB;
...
CREATE TABLE Usuarios (
    ...
    CONSTRAINT FK_Usuarios_TiposUsuario FOREIGN KEY (id_tipo_usuario)
        REFERENCES TiposUsuario(id_tipo_usuario)
        ON DELETE RESTRICT ON UPDATE CASCADE,
...
CREATE TABLE LivrosAutores (
    ...
    CONSTRAINT FK_LivrosAutores_Livros FOREIGN KEY (id_livro)
        REFERENCES Livros(id_livro)
        ON DELETE CASCADE ON UPDATE CASCADE,

    CONSTRAINT FK_LivrosAutores_Autores FOREIGN KEY (id_autor)
...
CREATE TABLE Exemplares (
    ...
    status ENUM('Disponível', 'Emprestado', 'Reservado', 'Manutenção', 'Perdido') DEFAULT 'Disponível' NOT NULL,
    data_aquisicao DATE,
    localizacao VARCHAR(50),

    CONSTRAINT FK_Exemplares_Livros FOREIGN KEY (id_livro)
        REFERENCES Livros(id_livro)
        ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB;
...
CREATE TABLE Emprestimos (
    ...
    CONSTRAINT FK_Emprestimos_Exemplares FOREIGN KEY (id_exemplar)
        REFERENCES Exemplares(id_exemplar)
        ON DELETE RESTRICT ON UPDATE CASCADE,

    CONSTRAINT CHK_data_prevista CHECK (data_prevista_devolucao >= DATE(data_emprestimo)),
    CONSTRAINT CHK_data_devolucao_real CHECK (data_devolucao_real IS NULL OR data_devolucao_real >= data_emprestimo)
) ENGINE=InnoDB;
...
CREATE TABLE Multas (
    ...
    CONSTRAINT FK_Multas_Emprestimos FOREIGN KEY (id_emprestimo)
    ...
    CONSTRAINT CHK_data_pagamento CHECK (data_pagamento IS NULL OR data_pagamento >= data_geracao)
) ENGINE=InnoDB;
...
CREATE TABLE Reservas (
    ...
    CONSTRAINT FK_Reservas_Livros FOREIGN KEY (id_livro)
        REFERENCES Livros(id_livro)
        ON DELETE CASCADE ON UPDATE CASCADE,

    CONSTRAINT CHK_data_validade CHECK (data_validade IS NULL OR data_validade >= DATE(data_reserva))
) ENGINE=InnoDB;
...
INSERT INTO TiposUsuario (nome_tipo, max_emprestimos, prazo_dias) VALUES
('Aluno', 3, 14),
('Professor', 5, 30),
('Funcionário', 4, 21),
('Visitante', 1, 7);
